...
namespace IndusG.BackgroundServiceImplement
{
    class modbus
    {
        ...
        public bool Open(string portName, int baudRate, int databits, Parity parity, StopBits stopBits)
        {
            ...
            if (!sp.IsOpen)
            {
                ...
                {
                    LoggerHelper.Error("Error opening " + portName + ": " + err.Message);
                    return false;
                }
        ...
        public bool SendFc3(byte address, ushort start, ushort registers, ref short[] values)
        {
            ...
            if (sp.IsOpen)
            {
                ...
                {
                    LoggerHelper.Error("Error in read event: " + err.Message);
                    return false;
                }
                //Evaluate message:
                if (CheckResponse(response))
                ...
                {
                    LoggerHelper.Error("CRC error");
                    return false;
                }
            }
            else
            {
                LoggerHelper.Error("Serial port not open");
                return false;
            }
