...
public partial class Dashboard : System.Web.UI.Page
{
	...
	string quanTracConnection = System.Configuration.ConfigurationManager.AppSettings["QuanTrac"];
	int companyID;
    ...
    protected void LoadData()
    {
        if (!Page.IsPostBack)
        {
            PlantService plantService = new PlantService(quanTracConnection);
			...
			if (Session["SelectedPlant_" + companyID] != null)
			{
				cmbPlants.SelectedValue = Session["SelectedPlant_" + companyID.ToString()].ToString();
			}
            LoadMeasurementInfo(int.Parse(cmbPlants.SelectedValue));
        }
    ...
    {
		Session["SelectedPlant_" + companyID] = plantID;
        MeasurementService measurementService = new MeasurementService(quanTracConnection);
